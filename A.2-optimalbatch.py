...
for batch_size in batch_sizes:
	...
	batch_train_acc[batch_size] = []
	batch_train_err[batch_size] = []
	batch_train_cross_entropy[batch_size] = []
	...
	with tf.Session() as sess:
		...
		for i in range(epochs):
			...
			for j in range(0,trainX.shape[0],batch_size):
				...
				temp_entropy.append(j_ent)
			
			batch_train_cross_entropy[batch_size].append(np.average(temp_entropy))
			batch_train_acc[batch_size].append(np.average(temp_acc))
			batch_train_err[batch_size].append(np.sum(temp_err))
			...
			if (i % 100 == 0) or (i == (epochs-1)):
				...
				print('iter %d: accuracy %g, error %g, entropy %g'%(i, batch_train_acc[batch_size][i],batch_train_err[batch_size][i],batch_train_cross_entropy[batch_size][i]))
...
for i in range(len(batch_sizes)):
	print('test_batch_size_%d: %g, err: %g'%(batch_sizes[i],batch_test_acc[i],batch_test_err[i])) 
...
for i in batch_train_acc.keys():
	plt.plot(range(epochs), batch_train_acc[i], label='batch_size = {}'.format(i))
...
#BATCH_SIZE/TEST_ACC
plt.figure(2)
plt.plot(range(len(batch_sizes)),batch_test_acc, label='batch_size = {}'.format(i))
# plt.plot(range(batch_test_acc.keys()), batch_test_acc[i], label='batch_size = {}'.format(i))
...
plt.figure(3)
plt.plot(range(len(batch_sizes)), batch_time_set)
plt.xticks(range(len(batch_sizes)), batch_sizes)
plt.xlabel('batch size')
plt.ylabel('time to train (min)')
plt.savefig('batch_3.png')

#BATCH_SIZE/TRAIN_TIME_SINGLE_EPOCH
plt.figure(4)
plt.plot(range(len(batch_sizes)), batch_single_epoch)
plt.xticks(range(len(batch_sizes)), batch_sizes)
plt.xlabel('batch size')
plt.ylabel('time to train (s)')
plt.savefig('batch_4.png')


#BATCH_SIZE/TRAIN_ERR
plt.figure(5)
for i in batch_train_err.keys():
	plt.plot(range(epochs), batch_train_err[i], label='batch_size = {}'.format(i))
plt.xlabel(str(epochs) + ' iterations')
plt.legend()
plt.ylabel('train errors')
plt.savefig('batch_5.png')

#BATCH_SIZE/TEST_ERR
plt.figure(6)
plt.plot(range(len(batch_sizes)),batch_test_err, label='batch_size = {}'.format(i))
plt.xlabel('batch size.')
plt.xticks(range(len(batch_sizes)), batch_sizes)
plt.ylabel('test error')
plt.savefig('batch_6.png')


# CROSS ENTROPY
plt.figure(7)
for i in batch_train_cross_entropy.keys():
	plt.plot(range(epochs),batch_train_cross_entropy[i],label='batch_size = {}'.format(i))
plt.xlabel('iterations.')
plt.ylabel('cross entropy')
plt.legend()
plt.savefig('batch_7.png')
plt.show()