...
learning_rate = 0.01
beta_array = [0,0.001,0.000001,0.000000001,0.000000000001]
epochs = 1000
batch_size = 16
num_neurons = 10
seed = 10
...
for beta in beta_array:
	...
	batch_train_acc[beta] = []
	batch_train_err[beta] = []
	batch_train_cross_entropy[beta] = []
	...
	with tf.Session() as sess:
		...
		for i in range(epochs):
			...
			batch_train_acc[beta].append(np.average(temp_acc))
			batch_train_err[beta].append(np.sum(temp_err))
			batch_train_cross_entropy[beta].append(np.average(temp_ent))

			if (i % 100 == 0) or (i == (epochs-1)):
				print('iter %d: accuracy %g, error %g, entropy %g'%(i, batch_train_acc[beta][i],batch_train_err[beta][i], batch_train_cross_entropy[beta][i]))
...
for i in range(len(beta_array)):
	print('test_beta_%d: %g, err: %g'%(beta_array[i],batch_test_acc[i],batch_test_err[i]))
...
for i in batch_train_acc.keys():
	plt.plot(range(epochs), batch_train_acc[i], label='beta = {}'.format(i))
...
#BATCH_SIZE/TEST_ACC
plt.figure(2)
plt.plot(range(len(beta_array)),batch_test_acc, label='beta = {}'.format(i))
plt.xlabel('beta')
plt.xticks(range(len(beta_array)), beta_array)
plt.ylabel('test accuracy')
plt.savefig('beta_2.png')

# #BATCH_SIZE/TRAIN_TIME
# plt.figure(3)
# plt.plot(range(len(beta_array)), batch_time_set)
# plt.xticks(range(len(beta_array)), beta_array)
# plt.xlabel('beta.')
# plt.ylabel('time to train (min)')
# plt.savefig('beta_3.png')

#BATCH_SIZE/TRAIN_ERR
plt.figure(4)
for i in batch_train_err.keys():
	plt.plot(range(epochs), batch_train_err[i], label='beta = {}'.format(i))
plt.xlabel(str(epochs) + ' iterations')
plt.legend()
plt.ylabel('train errors')
plt.savefig('beta_4.png')

#BATCH_SIZE/TEST_ERR
plt.figure(5)
plt.plot(range(len(beta_array)),batch_test_err, label='beta = {}'.format(i))
# plt.plot(range(batch_test_acc.keys()), batch_test_acc[i], label='beta = {}'.format(i))
plt.xlabel('beta.')
plt.xticks(range(len(beta_array)), beta_array)
plt.ylabel('test error')
plt.savefig('beta_5.png')

# CROSS ENTROPY
plt.figure(6)
for i in batch_train_cross_entropy.keys():
	plt.plot(range(epochs),batch_train_cross_entropy[i],label='beta = {}'.format(i))
plt.xlabel('iterations.')
plt.ylabel('cross entropy')
plt.savefig('beta_6.png')
plt.legend()
